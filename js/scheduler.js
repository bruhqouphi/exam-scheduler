...
const Scheduler = (function () {
  ...
  function buildContext(state, options) {
    ...
    const slots = state.slots.filter(s => s.available !== false)
      ...
      .sort((a, b) => a.date === b.date
        ? Store.toMinutes(a.start) - Store.toMinutes(b.start)
        : (a.date < b.date ? -1 : 1));
    ...
    for (let i = 0; i < slots.length; i++) {
      for (let j = 0; j < slots.length; j++) {
        ...
        if (Store.slotsOverlap(a, b)) overlaps[a.id].add(b.id);
        else if (Store.slotsAdjacent(a, b, opts.gapMinutes)) adjacent[a.id].add(b.id);
      }
    }
    ...
    return {
      ...
      slotById: id => Store.byId(state.slots, id),
      roomById: id => Store.byId(state.rooms, id)
    };
  }
  ...
  function buildDomains(courses, ctx) {
    ...
    courses.forEach(course => {
      const size = Store.headcount(course);
      const values = [];
      ...
      ctx.rooms.forEach(room => {
        ...
        ctx.slots.forEach(slot => {
          if (Store.slotLength(slot) < course.duration) return;
          slotFits = true;
  ...
  function place(board, course, value, ctx) {
    ...
    Store.studentsOf(course).forEach(st => {
  ...
  function unplace(board, course, value, ctx) {
    ...
    Store.studentsOf(course).forEach(st => drop(board.studentUse[st] || [], value.slotId));
  ...
  function isConsistent(board, course, value, ctx) {
    ...
    const students = Store.studentsOf(course);
  ...
  function evaluate(entries, courses, ctx) {
    const opts = ctx.opts;
    const byCourse = {};
      ...
      for (let i = 0; i < remaining.length; i++) {
        ...
        if (!pick || values.length < pickValues.length ||
            (values.length === pickValues.length &&
             Store.headcount(remaining[i]) > Store.headcount(pick))) {
          pick = remaining[i];
          pickValues = values;
        }
  ...
  function generate(state, options) {
    ...
    entries.sort((a, b) => {
      ...
      return sa.date === sb.date
        ? Store.toMinutes(sa.start) - Store.toMinutes(sb.start)
        : (sa.date < sb.date ? -1 : 1);
    });
  ...
  function detectConflicts(state, options) {
    const opts = Object.assign({}, DEFAULTS, options || {});
    const issues = [];
    ...
    resolved.forEach(x => {
      ...
      // H3 — capacity
      const size = Store.headcount(x.course);
    ...
    for (let i = 0; i < resolved.length; i++) {
      for (let j = i + 1; j < resolved.length; j++) {
        ...
        const overlap = sameSlot || Store.slotsOverlap(a.slot, b.slot);
        ...
        } else if (Store.slotsAdjacent(a.slot, b.slot, opts.gapMinutes)) {
          issues.push({
